...
const page = () => {
  ...
  const activeTemplate =
    ...
    templates[0];
  const activeFont = fonts.find((font) => font.id === selectedFont) ?? fonts[0];
      ...
      <div className="relative mx-auto flex min-h-svh w-full max-w-7xl flex-col px-5 sm:px-8 lg:px-12">
        ...
        >
          ...
          <p className="font-desc mx-auto mt-5 max-w-xl text-base leading-7 text-muted-foreground sm:text-lg">
            Choose a paper mood and a typeface, then let your feelings take up
            space.
          </p>
        </motion.section>
        ...
        <section className="mx-auto grid w-full max-w-5xl gap-6 pb-16 lg:grid-cols-[0.9fr_1.1fr] lg:items-start">
          <motion.div
            ...
            className="rounded-[1.5rem] border border-border bg-card/75 p-5 shadow-sm backdrop-blur sm:p-7"
          >
            <div className="mb-7">
            ...
            <form className="space-y-6" onSubmit={handleSubmit}>
              <div className="grid gap-5 sm:grid-cols-2">
                <div className="space-y-2">
                  ...
                  <input
                    ...
                    autoComplete="off"
                    className="flex h-11 w-full rounded-xl border border-input bg-background px-3 text-sm text-foreground outline-none transition-colors placeholder:text-muted-foreground/70 focus:border-ring focus:ring-2 focus:ring-ring/20"
                  />
                </div>
                <div className="space-y-2">
                  <label
                  ...
                  {templates.map((template) => {
                    ...
                    return (
                      ...
                      >
                        <span
                          aria-hidden="true"
                          className="absolute -right-2 -top-5 rotate-12 font-serif text-7xl opacity-10 transition-transform duration-300 group-hover:rotate-6"
                        >
                          {template.ornament}
                        </span>
                        <span className="relative flex items-center justify-between">
                          <span className="flex size-8 items-center justify-center rounded-full border border-current/20 bg-background/20">
                            <Heart className="size-3.5" />
                          </span>
                          <span
                            className={`flex size-5 items-center justify-center rounded-full border ${isSelected ? "border-primary bg-primary text-primary-foreground" : "border-current/25 text-transparent"}`}
                          >
                            <Check className="size-3" />
                          </span>
                        </span>
                        <span className="relative mt-5 block text-sm font-semibold">
                          {template.name}
                        </span>
                        <span className="relative mt-1 block text-xs opacity-70">
                  ...
                  {fonts.map((font) => {
                    ...
                    return (
                      <button
                        ...
                        aria-pressed={isSelected}
                        className={`flex items-center gap-3 rounded-xl border p-3 text-left transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 ${isSelected ? "border-primary bg-secondary/50" : "border-border bg-background hover:border-primary/40"}`}
                      >
                        <span
                          style={{ fontFamily: `var(${font.variable})` }}
                          className="flex size-10 shrink-0 items-center justify-center rounded-lg bg-muted text-lg text-primary"
                        >
                          Aa
                        </span>
                        ...
                        <span
                          className={`flex size-4 items-center justify-center rounded-full border ${isSelected ? "border-primary bg-primary text-primary-foreground" : "border-border text-transparent"}`}
                        >
              ...
              <div className="space-y-2">
                ...
                <textarea
                  ...
                  maxLength={1000}
                  rows={8}
                  placeholder="Write what your heart has been trying to say..."
                  className="w-full resize-none rounded-xl border border-input bg-background px-3 py-3 text-sm leading-6 text-foreground outline-none transition-colors placeholder:text-muted-foreground/70 focus:border-ring focus:ring-2 focus:ring-ring/20"
                />
              </div>
          ...
          >
            <div className="mb-4 flex items-center justify-between">
              <div>
                ...
                <h2 className="font-heading mt-2 text-2xl font-semibold tracking-tight">
                  Your letter, beautifully
                </h2>
              </div>
              <div className="hidden size-10 items-center justify-center rounded-full bg-accent text-accent-foreground sm:flex">
                <Heart className="size-4 fill-current" />
              </div>
            </div>
            ...
            <div
              ...
              className={`relative min-h-124 overflow-hidden border p-7 shadow-md transition-all duration-500 sm:p-10 ${activeTemplate.className} ${activeTemplate.previewClassName}`}
            >
              <div
                aria-hidden="true"
                className="absolute -right-10 -top-12 rotate-12 text-8xl text-primary/15"
              >
                {activeTemplate.ornament}
              </div>
              <div className="relative">
                <div className="mb-12 flex items-center justify-between border-b border-current/15 pb-5">
                  ...
                  <span className="text-xs opacity-60">From the heart</span>
                </div>
                <div className="space-y-7">
                  ...
                  <p className="max-w-full whitespace-pre-wrap wrap-break-words text-base leading-loose opacity-85">
                    {message ||
                      "Your heartfelt message will appear here as you write. Take your time and let your feelings guide every word."}
                  </p>
                  <div className="pt-5">
                    <p className="text-xl">{closing || "With all my love"},</p>
                    <p className="mt-2 text-sm opacity-60">
                      {sender || "Your name"}
                    </p>
                  </div>
                </div>
                <div className="absolute -bottom-20 right-0 rotate-12 text-7xl text-primary/15">
                  ♡
                </div>
              </div>
            </div>
            <p className="mt-4 text-center text-xs leading-5 text-muted-foreground">
              {activeTemplate.name} · {activeFont.name}
            </p>
          </motion.div>
        </section>
